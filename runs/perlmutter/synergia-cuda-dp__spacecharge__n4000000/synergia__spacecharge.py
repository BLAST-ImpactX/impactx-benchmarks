...
# Warm up on the SAME simulator object we then time. The SC workspaces + cuFFT plan are cached on the
# bunch-simulator's id, so warming a *different* sim (the old code did) left them to be (re)built
# INSIDE the timed region -- an unfair fixed overhead ImpactX does not pay. Warm at the full npart,
# then re-populate the SAME bunch to the identical initial Gaussian so the timed propagate is a clean
# single solve (same id -> plan/workspaces stay cached).
sim = make_sim(npart)
bunch = sim.get_bunch()
...
with Timer() as t:
    ...
    # Force the async CUDA map/solve kernels to finish WITHOUT an O(N) device->host copy: an on-device
    # reduction (parallel_reduce + Kokkos fence, 6 doubles out) is the cheap end-to-end sync a fair,
    # ImpactX-comparable timing needs. The full checkout (host copy, for observables) is AFTER the timer.
    synergia.bunch.Core_diagnostics.calculate_mean(bunch)
