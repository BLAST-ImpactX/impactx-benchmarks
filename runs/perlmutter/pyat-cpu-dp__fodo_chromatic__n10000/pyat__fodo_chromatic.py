#!/usr/bin/env python3
# Auto-generated benchmark run script: pyAT / fodo_chromatic (QuadLinearPass + DriftPass; chromatic-paraxial).
import json
...
#  - DriftPass: chromatic-paraxial drift x += L*px/(1+delta) (consistent with the quad).
ring = at.Lattice(
    [
        at.Quadrupole("QF", p["quad_length"], p["k1"], PassMethod="QuadLinearPass"),
        at.Drift("D1", p["drift_length"], PassMethod="DriftPass"),
        at.Quadrupole("QD", p["quad_length"], -p["k1"], PassMethod="QuadLinearPass"),
        at.Drift("D2", p["drift_length"], PassMethod="DriftPass"),
    ],
    energy=energy_eV,
    periodicity=1,
)
