#!/usr/bin/env python3
# Auto-generated benchmark run script: PyORBIT3 / spacecharge_2p5d (2.5D PIC).
# SpaceChargeCalc2p5D: transverse 2D FFT Poisson per longitudinal slice (2.5D), open
# transverse boundary (boundary=None). One SC kick over the drift.
import json
import math
import time
...
def make_lattice():
    lat = TEAPOT_Lattice("sc2p5d")
    d = DriftTEAPOT("d1")
    d.setLength(p["drift_length"])
    d.setnParts(1)
    lat.addNode(d)
    lat.initialize()
    calc = SpaceChargeCalc2p5D(n, n, n)
    setSC2p5DAccNodes(lat, p["drift_length"], calc)  # boundary=None -> open transverse BC
    return lat
