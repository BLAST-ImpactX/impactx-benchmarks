"""
Define the BELLA HTU beamline lattice for all benchmark codes.

Model: chromatic-paraxial quadrupoles (k1 -> k1/(1+delta)) + chromatic-paraxial drifts, the
fast workhorse model. The few weak chicane bends use each code's thick-bend model (a secondary
element; differences are within the htu tolerance). Cheetah has no chromatic quad/drift and
SciBmad no chromatic drift, so they run the costlier exact map (flagged with an asterisk via
the htu scenario's ``untuned_codes``).
"""
...
try:
    ...
    pyat_available = False
# Try to import Xsuite
try:
    import xtrack as xt
    xsuite_available = True
except ImportError:
    xsuite_available = False
# Try to import PyORBIT3
try:
    from orbit.teapot import DriftTEAPOT, QuadTEAPOT, BendTEAPOT
    pyorbit_available = True
except ImportError:
    pyorbit_available = False


tracking_method = {
    "cheetah": "drift_kick_drift",  # symplectic chromatic (formerly "bmadx" in older Cheetah)
    "cheetah-linear": "linear",
}
...
def get_lattice( code,
    ...
    Parameters:
        code (str):
            "impactx", "cheetah" (uses tracking_method="drift_kick_drift") or "cheetah-linear" (uses tracking_method="linear")
...
def screen( name, code, experiment_configuration_file=None, experiment_analysis_settings=True, as_marker=False ):
    ...
    elif code == "pyat" and pyat_available:
        return at.Marker(name)  # screens benchmarked as markers
    elif code == "xsuite" and xsuite_available:
        return xt.Marker()
    elif code == "pyorbit" and pyorbit_available:
        m = DriftTEAPOT(name); m.setLength(0.0); return m  # zero-length no-op marker
    elif code == "spec":
        return {"kind": "marker", "name": name}
    else:
...
def quadrupole( name, L, k1=None, current=None, design=None, bore_radius=None, B=None, code=None, reference_energy_eV=100e6 ):
    ...
    if code.startswith("cheetah") and cheetah_available:
        ...
        k1 = Bgradient / get_rigidity(reference_energy_eV)
        return at.Quadrupole(name, L, k1, PassMethod="QuadLinearPass")  # exact thick chromatic
    elif code == "xsuite" and xsuite_available:
        k1 = Bgradient / get_rigidity(reference_energy_eV)
...
def drift( name, L, code ):
    ...
    HTU is a chromatic-paraxial benchmark (chromatic quads), so the drifts use the matching
    chromatic-paraxial drift where available. Cheetah has only the exact (drift_kick_drift)
    drift and SciBmad (via the "spec" path) only the exact drift -- those run the costlier
    exact map (flagged with an asterisk in the htu scenario's untuned_codes).
    """
    if code.startswith("cheetah") and cheetah_available:
        return Drift(name=name, length=torch.tensor(L), tracking_method=tracking_method[code])
    elif code == "impactx" and impactx_available:
        return elements.ChrDrift(name=name, ds=L, nslice=1)  # chromatic-paraxial
    elif code == "pyat" and pyat_available:
        return at.Drift(name, L, PassMethod="DriftPass")  # chromatic-paraxial
    elif code == "xsuite" and xsuite_available:
        return xt.Drift(length=L, model="expanded")  # chromatic-paraxial
    elif code == "pyorbit" and pyorbit_available:
        d = DriftTEAPOT(name); d.setLength(L); return d  # chromatic-paraxial (TEAPOT)
    elif code == "spec":
        return {"kind": "drift", "name": name, "L": L}  # SciBmad: exact-only drift
    else:
        raise ValueError(f"Unsupported code: {code}")
...
def kicker( name, current_h, current_v, max_current, max_integrated_field, code, reference_energy_eV=100e6 ):
    ...
    elif code in ("pyat", "xsuite", "pyorbit"):
        # HTU benchmark uses zero corrector currents -> these are no-ops; represent as
        # markers (a real corrector element would be needed for nonzero currents).
        if code == "pyat" and pyat_available:
            return at.Marker(name)
        if code == "xsuite" and xsuite_available:
            return xt.Marker()
        if code == "pyorbit" and pyorbit_available:
            k = DriftTEAPOT(name); k.setLength(0.0); return k
        raise ValueError(f"Unsupported code: {code}")
    elif code == "spec":
        return {"kind": "marker", "name": name}
    else:
...
def dipole( name, L, angle=None, r56=None, bend=None, code=None, reference_energy_eV=100e6):
    ...
    if code.startswith("cheetah") and cheetah_available:
        ...
        return at.Dipole(name, L, angle)  # BndMPoleSymplectic4Pass (symplectic bend)
    elif code == "xsuite" and xsuite_available:
        return xt.Bend(length=L, angle=angle)  # thick bend
    elif code == "pyorbit" and pyorbit_available:
